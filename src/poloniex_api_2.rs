...
const MAX_VOLUME_CHANGES:usize = 10;
const MAX_PRICE_CHANGES:usize = 10;
const MAX_PERCENTAGE_CHANGES:usize = 10;
...
#[derive(Clone)]
pub struct State {
  pub prices: HashMap<PairName, Price>,
  pub percentages: HashMap<PairName, Percentage>,
  pub order_books: HashMap<PairName, OrderBook>
}
...
pub type Volume = f64;
pub type SellBuy = (Volume, Volume);

#[derive(Clone)]
pub struct Price {
  pub changes: Vec<SellBuy>
}
...
pub struct OrderBook {
  pub sell: Volume,
  pub buy: Volume,
  pub depth: Vec<HashMap<String,String>>,
  pub changes: Vec<SellBuy>
}
...
impl OrderBook {
  pub fn new() -> OrderBook {
    OrderBook {
      sell: 0.0,
      buy: 0.0,
      depth: vec![HashMap::new(), HashMap::new()],
      changes: vec![]
    }
  }

  pub fn depth_totals(depth: &HashMap<String, String>) -> Volume {
    depth
      .values()
  ...
  pub fn recalculate(self: &mut Self) {
    self.sell = OrderBook::depth_totals(&self.depth[0]);
    self.buy = OrderBook::depth_totals(&self.depth[1]);
  }
}
...
impl MarketEvent {
	pub fn handle(market_id: &String, json: &Value, state: &mut State) {
	...
	fn handle_initial_sync(market_id: &String, data: &Value, state: &mut State) {
		...
		order_book.depth[0] = serde_json::from_value::<HashMap<String, String>>(sell).unwrap();
		order_book.depth[1] = serde_json::from_value::<HashMap<String, String>>(buy).unwrap();
		order_book.recalculate();
	}

	fn handle_order_modify(market_id: &String, data: &Value, state: &mut State) {
		// 1 is sell or buy
		// 2 is btc price
		// 3 is volume
		let order_book = state.order_books
		...
		let bid_ask_type = data[1].to_string().parse::<usize>().unwrap();
    if bid_ask_type == 0 {
      order_book.changes.push((amount.parse::<Volume>().unwrap(), 0.0));
    } else {
      order_book.changes.push((0.0, amount.parse::<Volume>().unwrap()));
    }

    order_book.depth[bid_ask_type]
      .insert(serde_json::from_value::<String>(data[2].clone()).unwrap(), amount);
		order_book.recalculate();
  }

	fn handle_new_trade(market_id: &String, data: &Value, state: &mut State) {
		// 2 is sell or buy
		// 3 is btc price
		// 4 is volume
		let order_book = state.order_books
		...
		let amount = serde_json::from_value::<String>(data[4].clone()).unwrap();
		let bid_ask_type = data[2].to_string().parse::<usize>().unwrap();
    order_book.depth[bid_ask_type]
      .insert(serde_json::from_value::<String>(data[3].clone()).unwrap(), amount);
		order_book.recalculate();
	}
}
...
impl TickerEvent {
  pub fn handle(market_id: &String, json: &Value, state: &mut State) {
   let lowest_ask = serde_json::from_value::<String>(json[2][2].clone()).unwrap().to_string().parse::<Volume>().unwrap();
   let highest_bid = serde_json::from_value::<String>(json[2][3].clone()).unwrap().to_string().parse::<Volume>().unwrap();
   let percentage = serde_json::from_value::<String>(json[2][4].clone()).unwrap().to_string().parse::<Volume>().unwrap() * (100.0 as Volume);
   let prices = state
    .prices
    .entry(market_id.clone())
    .or_insert(Price { changes: vec![] });

   prices.changes.push((lowest_ask, highest_bid));
   if prices.changes.iter().count() > MAX_PRICE_CHANGES {
    prices.changes.remove(0);
   }

   let percentages = state
    .percentages
    .entry(market_id.clone())
    .or_insert(Percentage { changes: vec![] });
   percentages.changes.push(percentage);
   if percentages.changes.iter().count() > MAX_PERCENTAGE_CHANGES {
    percentages.changes.remove(0);
   }
  }
}
